...
int main(){
    ...
    cleanPixel();
    draw_sprite();
    file_name = "original.ppm";
    if (!save_pixels()){
        printf("ERROR saving original image\n");
        return 1;
    }
    ...
    file_name = "scale.ppm";
    if (!save_pixels()){
        printf("ERROR saving scale image\n");
        return 1;
    }
    ...
    file_name = "translation.ppm";
    if (!save_pixels()){
        printf("ERROR saving translation image\n");
        return 1;
    }
    
    // 3. Transformación de rotación
    cleanPixel();
    draw_sprite();
    rotation_transform(-15);
    file_name = "rotation.ppm";
    if (!save_pixels()){
        printf("ERROR saving rotation image\n");
        return 1;
    }
    
    // 4. Composición: rotación sobre la esquina inferior izquierda
    cleanPixel();
    draw_sprite();
    composite_transform_bottom_left(3, 24, 24, -15);
    file_name = "composite_bottom_left.ppm";
    if (!save_pixels()){
        printf("ERROR saving composite bottom left image\n");
        return 1;
    }
    
    // 5. Composición: rotación sobre el centro del sprite
    cleanPixel();
    draw_sprite();
    composite_transform_center(3, 24, 24, -15);
    file_name = "composite_center.ppm";
    if (!save_pixels()){
        printf("ERROR saving composite center image\n");
        return 1;
    }
    
    // 6. Composición usando la función de transformaciones simples (rotación sobre el origen)
    cleanPixel();
    draw_sprite();
    apply_simple_transformations(3, 24, 45, -15);
    file_name = "composite_simple.ppm";
    if (!save_pixels()){
        printf("ERROR saving composite simple image\n");
        return 1;
    }
